...
(let* ((T  (vector->list (read-ply-polygon-mesh (cadr (command-line)))))
       ...
       (M2 (make-material-linefill-fn
	     (lambda (z normal set-colour set-lw fill stroke)
	       (let* ((c (colour-hsv-gradient (make-colour 'hsva 1.0 1.0 0.2 0.5)
				     	      (make-colour 'hsva 0.6 0.5 1.0 0.5)))
		      (r (abs (a-dot normal (:> 0 0 1)))))
		 (set-colour (c r)) (fill) (set-lw 0.0001)
		 (stroke)))))
...
;       (P (map ($ polygon-add-material -- M1) T))
  ...
  (let-values (((A B) (split-by-sphere (make-sphere (:. -0.0124 0.0382 0.0149) 0.1) T)))
